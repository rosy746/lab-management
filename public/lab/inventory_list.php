...
@media(max-width:680px){
  ...
  .topbar-logo { width:36px; height:36px; font-size:1.1rem; border-radius:10px; }
  .topbar-text h1 { font-size:1rem; }
  .topbar-text span { font-size:.7rem; }
  .topbar-right { 
    width:100%; 
    margin-top:.3rem;
  ...
  .filter-group label { font-size:.74rem; }
  .filter-group select, 
  .filter-group input { 
    padding:.6rem .7rem; 
    font-size:.8rem;
  ...
  <div class="filter-box">
    <form method="GET" action="">
      ...
      <div class="filter-grid">
        ...
        <div class="filter-group">
          <label>Cari</label>
          <input type="text" name="search" placeholder="Nama, brand, model, serial..." value="<?= htmlspecialchars($search) ?>">
        </div>
      </div>
  ...
  <?php else: ?>
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          <?php foreach($inventories as $index => $item): ?>
            <tr>
              <td data-label="No" style="text-align:center;font-weight:600;color:var(--text-mid);"><?= $index + 1 ?></td>
              <td data-label="Nama Item"><strong style="color:var(--text-dark);"><?= htmlspecialchars($item['item_name']) ?></strong></td>
              <?php if($lab_filter==='all'): ?>
                <td data-label="Lab"><?= htmlspecialchars($item['lab_name']) ?></td>
              <?php endif; ?>
              <td data-label="Kategori"><span class="badge badge-cat-<?= htmlspecialchars($item['category']) ?>"><?= ucfirst(htmlspecialchars($item['category'])) ?></span></td>
              ...
              <td data-label="Serial Number"><?= htmlspecialchars($item['serial_number'] ?? '–') ?></td>
              <td data-label="Spesifikasi"><?= htmlspecialchars($item['specifications'] ?? '–') ?></td>
              ...
              <td data-label="Kondisi"><span class="badge badge-cond-<?= htmlspecialchars($item['condition']) ?>"><?= ucfirst(htmlspecialchars($item['condition'])) ?></span></td>
              <td data-label="Status"><span class="badge badge-st-<?= htmlspecialchars($item['status']) ?>"><?= ucfirst(htmlspecialchars($item['status'])) ?></span></td>
              <td data-label="Catatan"><?= htmlspecialchars($item['notes'] ?? '–') ?></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
